//todo: 1. class DataTimeFormats
//todo: 2. class AlertMessages
//todo: 3. class BottomSheets

import 'package:flutter/material.dart';
import 'package:intl/intl.dart';

//Photos Resource
final myPhotoGalleryOne = [
  {
    "img":
...
class ResponsiveComponents {
  /*String? imageAddress;
   ResponsiveComponents({this.imageAddress});*/
  Stack responsiveImageCardOne({imageAddressForMethod}) {
    return Stack(
      children: [
        const SizedBox(
  ...
  dynamic responsiveImageGrid() {
    return Center(
      child: AspectRatio(
        aspectRatio: 16 / 16,
        child: GridView.builder(
            scrollDirection: Axis.horizontal,
            itemCount: myPhotoGalleryOne.length,
            ...
            itemBuilder: (context, index) {
              return Card(
                elevation: 20,
                child: Image.network("${myPhotoGalleryOne[index]["img"]}"),
              );
            }),
      ),
    );
  ...
  dynamic responsiveImageCardTwo() {
    return Card(
      child: AspectRatio(
        aspectRatio: 1,
...
class DateTimeFormats {
  formattedDateTimeStyleOne(dateTimeFormation, context) {
    return DateFormat("dd-MM-yyyy, hh:mma").format(dateTimeFormation.dateTime);
  }
...
//Alert Message for all kinds

class AlertMessages {
  mySnackBarOne(message, context) {
    return ScaffoldMessenger.of(context)
        .showSnackBar(SnackBar(content: Text(message)));
  }

  showDialogOne(context) {
    return showDialog(
        ...
        builder: (context) {
          return AlertDialog(
            ...
            content: const Text("You've bought 5 Product"),
            actions: [
              TextButton(
                onPressed: () {
  ...
  myAlertDialogOne(context) {
    return showDialog(
        context: context,
        builder: (BuildContext context) {
          return Expanded(
            child: AlertDialog(
              title: const Text("Alert___!"),
              content: const Text("Do you want to delete?"),
              actions: [
                TextButton(
                    onPressed: () {
                      Navigator.of(context).pop();
                      mySnackBarOne("The book has been deleted...", context);
                    },
